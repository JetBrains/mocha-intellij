...
function finishTestNode(tree, test, err, finishingQueue) {
  ...
  if (err) {
    var errInfo = extractErrInfo(err);
    var expectedStr = getOwnProperty(err, 'expected');
    var actualStr = getOwnProperty(err, 'actual');
    if (expectedStr != null && actualStr != null) {
      expectedStr = stringifier.stringify(expectedStr);
      actualStr = stringifier.stringify(actualStr);
    }
    if (!util.isString(expectedStr) || !util.isString(actualStr)) {
      expectedStr = null;
      actualStr = null;
    }
