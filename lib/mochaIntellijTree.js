...
Node.prototype.finish = function (finishParentIfLast) {
  ...
  this.tree.writeln(text);
  if (text.indexOf("##teamcity[testFailed ") === 0) {
    this.tree.writeln("##teamcity[testFinished nodeId=\'" + this.id + "\']");
  }
 ...
 * @param expectedFilePath {string} path to file with expected data
 * @param actualFilePath {string} path to file with actual data
 * @public
 */
